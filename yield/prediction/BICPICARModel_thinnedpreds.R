...
res_delta<- resThin[,ncol(XMat)+1:rankM] 
res_s2<- resThin[,ncol(resThin)-1]
res_tau2<- resThin[,ncol(resThin)]
rm(XMat,XMatCV,res)
...
#for(k in 1:length(modelnames)){
  ...
  for (l in 1:2){
    ...
    for(t in 1:nfutureyrs){
      ...
      thinPreds[(nCounties*(t-1)+1):(nCounties*t),]<- predmat_test
    }

    save(thinPreds,file=paste("Climate_CornYield-me/yield/PICAR/timeConstant/noCIs/cutoff.2/rank20/output/",modelnames[k],"_",yr,"_thinPredMat.RData",sep=""))
...
time<- Sys.time()- time1
save(time,file=paste("Climate_CornYield-me/yield/PICAR/timeConstant/noCIs/cutoff.2/rank20/output/futurethinpredtime.RData",sep=""))
